...
function App() {
  ...
  async function fetchTrends() {
    ...
    setError(null);
    try {
      const res = await fetch(`https://25990a85a36a.ngrok-free.app/api/latest-trends/`);
  ...
  async function scrapeSaveTrend() {
    ...
    setError(null);
    try {
      const res = await fetch(`https://25990a85a36a.ngrok-free.app/api/scrape-save-trend/`, { method: "POST" });
      if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
      const data = await res.json();
      setTrends(data);
    } catch (err) {
